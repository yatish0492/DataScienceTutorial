...
SVM draws a line between categories. Then based on that it will indentify which category the input item falls in.
eg: if we give input as picture of Apple, then SVM will identity the fruit type is apple. If we give input as picture of Orange,
    then SVM will identify the fruit type as Orange.

NOTE: Same as that of 'Logistic Regression' code, we just need to update fitting code. even the data is same used in 
        'Logistic Regression'.
'''
...
classifier = SVC(kernel = 'linear', random_state = 0) # there are many kernels like 'linear','poly','rbf','sigmoid','precomputed' etc.
